    ...
    public async Task<List<PackageRecord>> GetPackagesInTimeRangeAsync(DateTime startTime, DateTime endTime)
    {
        ...
        while (currentDate <= endTime.Date)
        {
            try
            {
                ...
                string tableName = $"Packages_{currentDate:yyyyMMdd}";
                ...
                if (await dbContext.Database.CanConnectAsync())
                {
                    ...
                    if (efRecords.Count == 0)
                    {
                        ...
                        string startStr = startTime.ToString("yyyy-MM-dd HH:mm:ss.fff");
                        string endStr = endTime.ToString("yyyy-MM-dd HH:mm:ss.fff");
                        
                        string sql = $"SELECT * FROM {tableName} WHERE CreateTime BETWEEN @startTime AND @endTime ORDER BY CreateTime DESC";
