...
public class PendulumSortService : IPendulumSortService
{
    ...
    private readonly ConcurrentDictionary<string, PackageInfo> _pendingSortingActions = new();
    ...
    public async Task InitializeAsync(SortConfiguration configuration)
    {
        try
        {
            ...
            // 初始化分拣光电状态
            foreach (var photoelectric in _configuration.SortingPhotoelectrics)
            {
            ...
            foreach (var photoelectric in _configuration.SortingPhotoelectrics)
            {
                ...
                try
                {
                    ...
                    Log.Debug("已发送启动命令到分拣光电 {Name}", photoelectric.Name);
                    ...
                    await client.SendAsync(resetLeftCommand);
                    await client.SendAsync(resetRightCommand);
                    Log.Debug("已发送左右回正命令到分拣光电 {Name}", photoelectric.Name);

                    // 保存客户端连接
                    _sortingClients[photoelectric.Name] = client;

                    // 更新分拣光电状态
                    UpdateDeviceConnectionState(photoelectric.Name, true);
                }
                catch (OperationCanceledException)
                {
                    Log.Error("连接分拣光电 {Name} 超时", photoelectric.Name);
                    UpdateDeviceConnectionState(photoelectric.Name, false);
                    throw new TimeoutException($"连接分拣光电 {photoelectric.Name} 超时");
                }
                catch (Exception ex)
                {
                    Log.Error(ex, "连接分拣光电 {Name} 失败", photoelectric.Name);
                    UpdateDeviceConnectionState(photoelectric.Name, false);
                    throw;
                }
            }
            ...
            _ = Task.Run(async () =>
            {
                try
                {
                    while (!_cancellationTokenSource.Token.IsCancellationRequested)
                        ...
                        {
                            // 读取分拣光电数据
                            var sortingData = await client.Value.ReceiveAsync();

                            // 处理分拣光电数据
                            ProcessSortingData(sortingData, client.Key);

                            await Task.Delay(100, _cancellationTokenSource.Token);
                        }
                        catch (Exception ex)
                        {
                            Log.Error(ex, "处理分拣光电 {Name} 数据时发生错误", client.Key);
                ...
                {
                    Log.Error(ex, "分拣光电 {Name} 数据接收任务发生错误", client.Key);
                }
            }, _cancellationTokenSource.Token);
        }
    ...
    private void ProcessSortingData(byte[] data, string photoelectricName)
    {
        try
        {
            ...
            // 处理分拣光电的+OCCH1:1和+OCCH1:0信号
            if (message.Contains("+OCCH1:1"))
            {
                Log.Information("收到分拣光电 {Name} 触发信号，开始处理分拣逻辑", photoelectricName);
                ProcessSortingLogic(photoelectricName);
            }
            else if (message.Contains("+OCCH1:0"))
            {
                Log.Information("收到分拣光电 {Name} 复位信号，开始执行分拣动作", photoelectricName);
                ExecutePendingSortingAction(photoelectricName);
            }
        }
        catch (Exception ex)
        {
    ...
    private void ProcessSortingLogic(string photoelectricName)
    {
        try
        {
            // 获取当前时间
            ...
            packages = [.. packages.OrderBy(p => p.TriggerTimestamp).ThenBy(p => p.Index)];
            ...
            foreach (var package in packages)
            {
                ...
                // 验证包裹是否属于当前分拣光电处理
                var photoelectric = _configuration.SortingPhotoelectrics.FirstOrDefault(p => p.Name == photoelectricName);
                if (photoelectric == null)
                {
                    Log.Error("未找到分拣光电 {Name} 的配置", photoelectricName);
                    continue;
                }

                // 获取当前分拣光电的索引（从1开始）
                var photoelectricIndex = _configuration.SortingPhotoelectrics.ToList().IndexOf(photoelectric) + 1;
                // 计算当前分拣光电负责的格口范围
                var startChute = (photoelectricIndex - 1) * 2 + 1;
                var endChute = photoelectricIndex * 2;

                // 根据包裹的ChuteName判断是否属于当前分拣光电处理
                if (package.ChuteName < startChute || package.ChuteName > endChute)
                {
                    Log.Debug("包裹 {Barcode}(序号:{Index}, 格口:{Chute}) 不属于分拣光电 {Name}(负责格口:{Start}-{End})，跳过",
                        package.Barcode, package.Index, package.ChuteName,
                        photoelectricName, startChute, endChute);
                    continue;
                }

                Log.Debug("包裹 {Barcode}(序号:{Index}, 格口:{Chute}) 属于分拣光电 {Name}(负责格口:{Start}-{End})，继续处理",
                    package.Barcode, package.Index, package.ChuteName,
                    photoelectricName, startChute, endChute);

                // 验证时间延迟
                var delay = (currentTime - package.TriggerTimestamp).TotalMilliseconds;
                if (delay < photoelectric.TimeRangeLower ||
                    delay > photoelectric.TimeRangeUpper)
                {
                ...
                Log.Information(
                    "分拣光电 {Name} 与包裹 {Barcode} (序号: {Index}) 匹配成功，等待复位信号后执行分拣",
                    photoelectricName, package.Barcode, package.Index);

                // 标记包裹为处理中
                MarkPackageAsProcessing(package.Index, photoelectricName);

                // 保存待执行的分拣动作
                _pendingSortingActions[photoelectricName] = package;
        ...
        {
            Log.Error(ex, "处理分拣光电 {Name} 分拣逻辑时发生错误", photoelectricName);
        }
    }

    private async void ExecutePendingSortingAction(string photoelectricName)
    {
        if (!_pendingSortingActions.TryRemove(photoelectricName, out var package))
        {
            Log.Warning("未找到分拣光电 {Name} 的待执行分拣动作", photoelectricName);
            return;
        }

        await ExecuteSortingAction(package, photoelectricName);
    }
    ...
    private async Task ExecuteSortingAction(PackageInfo package, string photoelectricName)
    {
        try
        {
            ...
            if (!isNoAction)
            {
                // 执行分拣动作
                state.SetSwing();
                await _sortingClients[photoelectricName].SendAsync(HexStringToByteArray(command!));
                Log.Information("发送分拣命令: {Command} 到分拣光电, 包裹:{Barcode}",
                    command, package.Barcode);
            }
            else
            {
                Log.Information("包裹 {Barcode} 未设置有效格口号，跳过摆动命令", package.Barcode);
            }
            ...
            if (!isNoAction)
            {
                ...
                {
                    // 根据下一个包裹的格口号判断是否需要回正
                    var nextIsNoAction = nextPackage.ChuteName <= 0;
                    var nextIsRightSwing = nextPackage.ChuteName % 2 == 0;
                ...
                if (needReset)
                {
                    ...
                    state.SetReset();
                    await _sortingClients[photoelectricName].SendAsync(HexStringToByteArray(resetCommand));
                    Log.Information(
                        "发送回正命令: {Command} 到分拣光电, 原因:{Reason}, 当前状态:{State}",
                        resetCommand,
                ...
                else
                {
                    if (nextPackage != null)
                        Log.Information(
                            "跳过回正 - 分拣光电保持 {Direction} 摆动状态，因为下一个包裹 {NextCode} 使用相同方向, 当前状态:{State}",
    ...
    private async Task ReconnectAsync()
    {
        try
        {
            ...
            foreach (var client in _sortingClients)
            {
                ...
                {
                    Log.Error(ex, "断开分拣光电 {Name} 连接时发生错误", client.Key);
                }
            }

            // 清空分拣光电客户端字典
            _sortingClients.Clear();
            ...
            foreach (var photoelectric in _configuration.SortingPhotoelectrics)
            {
                ...
                try
                {
                    ...
                    // 更新分拣光电状态
                    UpdateDeviceConnectionState(photoelectric.Name, true);
                    Log.Information("分拣光电 {Name} 重新连接成功", photoelectric.Name);
                }
                catch (OperationCanceledException)
                {
                    Log.Error("重新连接分拣光电 {Name} 超时", photoelectric.Name);
                    UpdateDeviceConnectionState(photoelectric.Name, false);
                    throw new TimeoutException($"重新连接分拣光电 {photoelectric.Name} 超时");
                }
                catch (Exception ex)
                {
                    Log.Error(ex, "重新连接分拣光电 {Name} 失败", photoelectric.Name);
                    UpdateDeviceConnectionState(photoelectric.Name, false);
                    throw;
                }
            }
