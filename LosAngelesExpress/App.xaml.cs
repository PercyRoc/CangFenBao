...
public partial class App
{
    ...
    private const string MutexName = @"Global\LosAngelesExpress_CangFenBao_App_Mutex";
    ...
    protected override void RegisterTypes(IContainerRegistry containerRegistry)
    {
        ...
        containerRegistry.RegisterSingleton<CainiaoSettingsViewModel>();
        // 注册菜鸟API服务
        containerRegistry.RegisterSingleton<ICainiaoApiService, CainiaoApiService>();
        containerRegistry.RegisterForNavigation<MainWindow, MainViewModel>();
        containerRegistry.RegisterForNavigation<CainiaoSettingsView, CainiaoSettingsViewModel>();
    ...
    private void TryDisposeService<TService>(string serviceName) where TService : class, IDisposable
    {
        ...
        {
            Log.Debug("服务 {ServiceName} ({ServiceType}) 未在容器中注册，跳过处置。", serviceName, typeof(TService).Name);
            return;
        }
        ...
        try
        {
            ...
            Log.Information("正在处置服务: {ServiceName} ({ServiceType})...", serviceName, typeof(TService).Name);
            service.Dispose();
            Log.Information("服务 {ServiceName} ({ServiceType}) 的处置操作已发起或完成。", serviceName, typeof(TService).Name);
        }
        catch (Exception ex)
        {
            Log.Warning(ex, "解析或处置服务 {ServiceName} ({ServiceType}) 时发生错误。该服务可能已被处置或在处置过程中出错。", serviceName, typeof(TService).Name);
        }
    }
