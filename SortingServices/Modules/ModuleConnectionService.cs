...
public class ModuleConnectionService(ISettingsService settingsService) : IModuleConnectionService
{
    ...
    private readonly ModelsTcpSettings _config = settingsService.LoadSettings<ModelsTcpSettings>();
    ...
    public void OnPackageReceived(PackageInfo package)
    {
        try
        {
            ...
            foreach (var (packageNumber, waitInfo) in _waitingPackages)
            {
                ...
                Log.Debug(
                    "尝试匹配包裹: 序号={PackageNumber}, 条码={Barcode}, 等待时间={TimeDiff}ms, 有效范围={MinTime}-{MaxTime}ms",
                    packageNumber, package.Barcode, timeDiff, _config.MinTime, _config.MaxTime);

                // 检查时间范围
                if (!(timeDiff >= _config.MinTime) || !(timeDiff <= _config.MaxTime))
                {
                    if (timeDiff < _config.MinTime)
                    {
                        Log.Debug("包裹等待时间 {TimeDiff}ms 小于最小等待时间 {MinTime}ms, 跳过匹配",
                            timeDiff, _config.MinTime);
                    }
                    else if (timeDiff > _config.MaxTime)
                    {
                        Log.Debug("包裹等待时间 {TimeDiff}ms 大于最大等待时间 {MaxTime}ms, 标记为超时",
                            timeDiff, _config.MaxTime);
        ...
        try
        {
            ...
            _ = Task.Run(async () =>
            {
                try
                {
                    try
                    {
                        Log.Debug("启动包裹等待超时任务: 序号={PackageNumber}, 最大等待时间={MaxTime}ms",
                            packageNumber, _config.MaxTime);

                        await Task.Delay(_config.MaxTime, waitInfo.TimeoutCts.Token);
                        ...
                        if (_waitingPackages.TryRemove(packageNumber, out _))
                        {
                            ...
                            Log.Warning("包裹等待超时: 序号={PackageNumber}, 最大等待时间={MaxTime}ms, 绑定条码={Barcode}",
                                packageNumber, _config.MaxTime, boundBarcode);
