            ...
            while (true)
            {
                ...
                foreach (var package in offlinePackages)
                {
                    ...
                    try
                    {
                        ...
                        await _offlinePackageService.MarkOfflinePackageAsRetryCompletedAsync(package.Barcode,
                            package.CreateTime, result.IsSuccess);
                        ...
                        else
                        {
                            failCount++;
                            var errorMessage =
                                $"离线包裹 {package.Barcode} 重试上传失败：Code={result.Code}, Value={result.ResponseCodeValue}, Msg='{result.Message}'";
                ...
                // 重试上传不显示通知，仅记录日志
                if (successCount > 0) Log.Information("重试上传成功 {SuccessCount} 个离线包裹", successCount);
                if (failCount > 0)
                {
                ...
                if (skippedCount > 0) Log.Information("因网络断开，本次跳过 {SkippedCount} 个离线包裹的处理", skippedCount);
