...
public class WildberriesApiService
{
    ...
    public async Task<(bool success, string errorMessage)> SendTareAttributesAsync(TareAttributesRequest request)
    {
        try
        {
            ...
            Log.Information("Sending TareAttributes request to Wildberries API: {RequestUrl}, Body: {RequestBody}",
                _settings.TareAttributesEndpoint, jsonRequest);
            ...
            Log.Error("TareAttributes request failed with status code {StatusCode}. Response Body: {ResponseBody}",
                response.StatusCode, responseBody);
            ...
            try
            {
                ...
                if (errorResponse?.Errors != null && errorResponse.Errors.Count > 0)
                {
                    ...
                    return (false,
                        $"API Error: {firstError.Message} (Code: {firstError.Error}, Detail: {firstError.Detail})");
                }
            }
