...
public class PlcSettingsViewModel : BindableBase
{
    ...
    private void LoadConfiguration()
    {
        try
        {
            var config = _settingsService.LoadConfiguration<PlcSettings>();
        ...
        catch (Exception ex)
        {
            ...
            _notificationService.ShowError("加载PLC配置失败", ex.Message);
        }
    }
    ...
    private void SaveConfiguration()
    {
        try
        {
            ...
            _notificationService.ShowSuccess("保存成功", "PLC配置已保存");
        }
        catch (Exception ex)
        {
            Log.Error(ex, "保存PLC配置时发生错误");
            _notificationService.ShowError("保存PLC配置失败", ex.Message);
        }
    }
} 