...
public class WaybillUploadService
{
    ...
    private static List<(string Barcode, float Weight, double Length, double Width, double Height, Image<Rgba32>? Image)> GenerateTestData(int count)
    {
        var random = new Random();
        var testData = new List<(string Barcode, float Weight, double Length, double Width, double Height, Image<Rgba32>? Image)>();
    ...
    public async Task<List<(string Barcode, ApiResponse InfoResponse, ApiResponse? ImageResponse)>> TestBatchUploadAsync(int count = 50)
    {
