...
namespace BalanceSorting.Service;
...
public abstract class BasePendulumSortService : IPendulumSortService
{
    ...
    ///     摆轮状态类
    /// </summary>
    protected class PendulumState(string photoelectricName) : IDisposable
    {
        private bool _isInReset = true;
        public int LastSlot { get; private set; }
        private Timer? _pendingResetTimer;
        ...
        public void CancelPendingReset()
        {
            ...
            Log.Debug("Pending reset timer cancelled for {PhotoelectricName}.", photoelectricName);
        }
        ...
        public void ScheduleDelayedReset(double interval, Func<Task<bool>> resetActionAsync)
        {
            ...
            Log.Debug("Scheduled delayed reset for {PhotoelectricName} in {Interval}ms.", photoelectricName, interval);
        }
        ...
        private async void OnPendingResetTimerElapsed(object? sender, ElapsedEventArgs e)
        {
            ...
            Log.Information("Pending reset timer elapsed for {PhotoelectricName}. Current state before action: {State}, LastSlot: {LastSlot}. Executing reset action.", 
                photoelectricName, GetCurrentState(), LastSlot);
            ...
            {
                Log.Information("Reset timer for {PhotoelectricName} elapsed, but pendulum is already in Reset state or action is null. No action taken.", photoelectricName);
                return;
            }
            
            if (_resetActionAsync == null)
            {
                Log.Warning("Reset timer for {PhotoelectricName} elapsed, but no reset action was defined.", photoelectricName);
            ...
            try
            {
                ...
                {
                    _isInReset = true; 
                    Log.Information("Delayed reset action successfully executed for {PhotoelectricName}. State is now Reset.", photoelectricName);
                }
                else
                {
                    Log.Warning("Delayed reset action failed for {PhotoelectricName}. State remains Swing (or previous state). LastSlot: {LastSlot}", photoelectricName, LastSlot);
                }
            }
            catch (Exception ex)
            {
                Log.Error(ex, "Exception during delayed reset action for {PhotoelectricName}. State remains Swing (or previous state). LastSlot: {LastSlot}", photoelectricName, LastSlot);
