...
internal class ModuleConnectionService : IModuleConnectionService
{
    ...
    // Rx 主题，用于对外暴露信号流
    private readonly Subject<ushort> _triggerSignalSubject = new();
    ...
    public IObservable<ushort> TriggerSignalStream { get; }
    ...
    public Task StopServerAsync()
    {
        try
        {
            ...
            _isRunning = false;
            _tcpListener?.Stop();
            _triggerSignalSubject.OnCompleted(); // 通知订阅者流已结束
    ...
    {
        if (_networkStream == null || !_connectedClient.Connected)
        {
    ...
    private Task ProcessPackageDataAsync(byte[] data)
    {
        try
        {
            // 记录接收时间戳
    ...
    private void ProcessPackageNumber(byte[] data)
    {
        try
        {
            // 解析包裹序号
            ...
            // 直接把信号推送到流中
            _triggerSignalSubject.OnNext(packageNumber);
        }
        catch (Exception ex)
        {
    ...
    private void ProcessFeedback(byte[] data)
    {
        try
        {
            // 解析包裹序号
