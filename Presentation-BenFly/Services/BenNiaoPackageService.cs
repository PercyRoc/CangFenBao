...
public class BenNiaoPackageService : IDisposable
{
    ...
    private async Task<string?> GetRealTimeSegmentCodeAsync(string waybillNum)
    {
        try
        {
            ...
            Log.Information("实时查询三段码使用的AppId: {AppId}, 分拨中心: {Center}", _config.BenNiaoAppId, _config.BenNiaoDistributionCenterName);
        ...
        try
        {
            ...
            while (!uploadSuccessful && retryCount < maxRetries)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    if (ex is Renci.SshNet.Common.SshConnectionException || ex is IOException)
                    {
