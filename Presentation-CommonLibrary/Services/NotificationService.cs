...
public class NotificationService : INotificationService
{
    ...
    public void ShowInformationWithToken(string message, string token)
    {
        Application.Current.Dispatcher.Invoke(() =>
        {
            Growl.Info(new GrowlInfo
    ...
    public void Unregister(string token, FrameworkElement element)
    {
        if (element is not Panel panel)
            throw new ArgumentException("元素必须是Panel类型", nameof(element));

        Application.Current.Dispatcher.Invoke(() => Growl.Unregister(token, panel));
    }
}