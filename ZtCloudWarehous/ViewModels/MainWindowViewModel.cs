...
internal class MainWindowViewModel : BindableBase, IDisposable
{
    ...
    private async Task InitializeSortServiceAsync()
    {
        try
        {
            ...
            // 加载分拣配置
            var configuration = _settingsService.LoadSettings<PendulumSortConfig>("PendulumSortConfig");
    ...
    private Task UpdateDeviceStatusesAsync()
    {
        try
        {
            ...
            if (_sortService.IsRunning())
            {
                ...
                // 更新触发光电状态
                var triggerStatus = DeviceStatuses.FirstOrDefault(static x => x.Name == "触发光电");
                ...
                // 更新分检光电状态
                foreach (var status in DeviceStatuses.Where(static x => x.Name != "相机" && x.Name != "触发光电"))
                {
            ...
            else
            {
                ...
                foreach (var status in DeviceStatuses.Where(static x => x.Name != "相机"))
                {
                    status.Status = "未连接";
