...
public class MainViewModel: BindableBase, IDisposable
{
    ...
    private readonly IAmericanQuickHandsApiService _apiService;
    ...
    public MainViewModel(
        ...
        IAmericanQuickHandsApiService apiService,
    ...
    private async Task ProcessPackageAsync(PackageInfo package)
    {
        ...
        try
        {
            ...
            // 3. 检查重量和尺寸
            var hasWeight = package.Weight > 0;
            var hasDimensions = package is { Length: > 0, Width: > 0, Height: > 0 };
            string status;
            string? errorMessage = null;

            switch (hasWeight)
            {
                case false when !hasDimensions:
                ...
                default:
                {
                    ...
                    {
                        // 4. 调用Swiftx API进行分拣机扫码
                        try
                        {
                            Log.Information("包裹 {Barcode}: 开始调用Swiftx API进行分拣机扫码", package.Barcode);

                            // 获取API设置
                            var apiSettings = _settingsService.LoadSettings<AmericanQuickHandsApiSettings>();

                            var scanRequest = new SortingMachineScanRequest
                            {
                                SortingMachineCode = apiSettings.SortingMachineCode,
                                TrackingNumber = package.Barcode,
                                WeightKg = package.Weight,
                                LengthCm = package.Length ?? 0,
                                WidthCm = package.Width ?? 0,
                                HeightCm = package.Height ?? 0
                            };

                            var scanResult = await _apiService.SortingMachineScanAsync(scanRequest);
                            if (scanResult.Success)
                            {
                                status = "API Success";
                                Log.Information("包裹 {Barcode}: 分拣机扫码成功: {Message}", package.Barcode, scanResult.Message);
                            }
                            else
                            {
                                status = "API Error: Sorting machine scan failed";
                                errorMessage = scanResult.Message ?? "Unknown error";
                                Log.Warning("包裹 {Barcode}: 分拣机扫码失败: {Error}", package.Barcode, errorMessage);
                            }
                        }
                        catch (Exception apiEx)
                        {
                            ...
                            Log.Error(apiEx, "包裹 {Barcode}: 调用Swiftx API时发生异常", package.Barcode);
                        }
                    }
                    break;
                }
