...
public sealed class HikvisionVolumeCameraService : ICameraService
{
    ...
    public IObservable<(float Length, float Width, float Height, DateTime Timestamp, bool IsValid, BitmapSource? Image)> VolumeDataWithVerticesStream
    {
        get => _volumeDataWithVerticesSubject.AsObservable();
    }
    ...
    private bool EnumerateDevicesInternal()
    {
        ...
        for (uint i = 0; i < _deviceList.nDeviceNum; i++)
        {
            try
            {
                ...
                var typeDesc = devInfo.nTLayerType switch
                {
    ...
    {
        var disconnectedSn = _currentCameraId;
        ...
        var deinitPerformedSuccessfully = true;
    ...
    public (bool IsSdkCallSuccess, float Length, float Width, float Height, bool IsMeasurementValid, BitmapSource? Image, Point[]? VertexPoints, string? ErrorMessage) GetSingleMeasurement()
    {
        ...
        try
        {
            ...
            if (ret != 0)
            {
                var errorDesc = GetHikvisionErrorDescription(ret);
    ...
    private static BitmapSource DrawRedBorderOnBitmapSource(BitmapSource originalImage, Point[] vertices)
    {
        ...
        using (var drawingContext = drawingVisual.RenderOpen())
        {
            ...
            {
                for (var i = 0; i < correctedVertices.Length - 1; i++)
                {
        ...
        var borderedBitmap = new RenderTargetBitmap(
    ...
    private static BitmapSource? ConvertFrameToBitmapSource(VOLM_FRAME_INFO frame)
    {
        ...
        try
        {
            ...
            var imageData = new byte[frame.nFrameLen];
            ...
            switch ((VOLM_PIXEL_TYPE)frame.enPixelType) // Assuming VOLM_PIXEL_TYPE is an enum
            {
                case VOLM_PIXEL_TYPE.PIXEL_TYPE_RGB8_PLANAR: // This might be PIXEL_TYPE_BGR8_PACKED or similar
                    ...
                    // 正确处理 RGB8 Planar 格式
                    var planeSize = frame.nWidth * frame.nHeight;
                    ...
                    var interleavedRgbData = new byte[planeSize * 3];

                    for (var y = 0; y < frame.nHeight; y++)
                    {
                        for (var x = 0; x < frame.nWidth; x++)
                        {
                            var planarPixelIndex = y * frame.nWidth + x; // 单个平面内的像素索引
                            var interleavedPixelStartIndex = planarPixelIndex * 3; // 交错数据中此像素的起始索引
