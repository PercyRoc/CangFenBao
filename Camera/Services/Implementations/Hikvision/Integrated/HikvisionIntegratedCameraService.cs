...
public class HikvisionIntegratedCameraService : ICameraService
{
    ...
    {
        get => _imageStreamWithIdSubject.AsObservable();
    }
    ...
    public bool Stop()
    {
        ...
        var nRetStop = _mvLogistics.MV_LGS_Stop_NET();
        ...
        var nRetDestroy = _mvLogistics.MV_LGS_DestroyHandle_NET();
    ...
    private void PackageCallBackFunc(IntPtr pstPkgInfo, IntPtr pUser)
    {
        ...
        try
        {
            var structure = Marshal.PtrToStructure(pstPkgInfo, typeof(MvLogistics.MVLGS_PACKAGE_INFOEx));
            if (structure == null)
            {
            ...
            if (stPackageInfoEx.bCodeEnable)
            {
                ...
                {
                    for (var j = 0; j < codeListItem.nCodeNum; ++j)
                    {
                        var nLength = Array.FindIndex(codeListItem.stCodeInfo[j].strCode, b => b == 0);
                        if (nLength == -1) nLength = codeListItem.stCodeInfo[j].strCode.Length;
                        var strBarCode = Encoding.UTF8.GetString(codeListItem.stCodeInfo[j].strCode, 0, nLength);
                    ...
                    if (!mainImageSet && codeListItem.stImage.nImageLen > 0 && codeListItem.stImage.pImageBuf != IntPtr.Zero)
                    {
                        ...
                        var imageFromCodeList = ConvertHikImageToBitmapSource(
    ...
    private void NoReadImageCallBackFunc(IntPtr noReadInfoPtr, IntPtr pUser)
    {
        ...
        try
        {
            var structure = Marshal.PtrToStructure(noReadInfoPtr, typeof(MvLogistics.MVLGS_IMAGE_OUTPUT_INFO));
            if (structure == null)
            {
    ...
    {
        var triggerStatus = pstTriggerInfo.nTriggerFlag == MvLogistics.MV_LGS_BEGIN_TRIGGER ? "开始触发" : "停止触发";
        ...
        try
        {
            ...
            switch (imageType)
            {
                case MvLogistics.MVLGS_IMAGE_TYPE.MVLGS_IMAGE_MONO8:
                    ...
                    bitmap = BitmapSource.Create(width, height, 96, 96, PixelFormats.Gray8, null, buffer, strideMono);
                    break;

                case MvLogistics.MVLGS_IMAGE_TYPE.MVLGS_IMAGE_BGR24:
                    var strideBgr = width * 3;
                    buffer = new byte[imageLen];
                    Marshal.Copy(imageDataPtr, buffer, 0, (int)imageLen);
                    bitmap = BitmapSource.Create(width, height, 96, 96, PixelFormats.Bgr24, null, buffer, strideBgr);
                    break;

                case MvLogistics.MVLGS_IMAGE_TYPE.MVLGS_IMAGE_RGB24:
                    var strideRgb = width * 3;
                    buffer = new byte[imageLen];
                    Marshal.Copy(imageDataPtr, buffer, 0, (int)imageLen);
                    bitmap = BitmapSource.Create(width, height, 96, 96, PixelFormats.Rgb24, null, buffer, strideRgb);
                    break;
