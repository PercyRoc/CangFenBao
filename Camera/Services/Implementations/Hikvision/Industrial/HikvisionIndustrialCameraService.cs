...
public sealed class HikvisionIndustrialCameraService : ICameraService
{
    ...
    {
        get => _imageWithIdSubject.AsObservable();
    }
    ...
    public bool Start()
    {
        ...
        try
        {
            ...
            {
                var enable1D = barcodeTypeSettings.BarcodeTypes.Any(bt => Is1DBarcodeType(bt.TypeId) && bt.IsEnabled);
                var enable2D = barcodeTypeSettings.BarcodeTypes.Any(bt => Is2DBarcodeType(bt.TypeId) && bt.IsEnabled);
    ...
    {
        var disconnectedSn = _currentDeviceSerialNumber;
    ...
    private bool EnumerateDevicesInternal()
    {
        ...
        for (uint i = 0; i < _enumeratedDeviceList.nDeviceNum; i++)
        {
            ...
            try
            {
                ...
                var serialNumber = deviceInfo.chSerialNumber ?? $"UnknownSN_{i}";
                var parsedUserDefinedName = Encoding.UTF8.GetString(deviceInfo.chUserDefinedName).TrimEnd('\0');
                var baseModelName = (deviceInfo.chModelName ?? string.Empty).TrimEnd('\0');
                ...
                var mac = $"{deviceInfo.nMacAddrHigh >> 8 & 0xFF:X2}:{deviceInfo.nMacAddrHigh & 0xFF:X2}:{deviceInfo.nMacAddrLow >> 24 & 0xFF:X2}:{deviceInfo.nMacAddrLow >> 16 & 0xFF:X2}:{deviceInfo.nMacAddrLow >> 8 & 0xFF:X2}:{deviceInfo.nMacAddrLow & 0xFF:X2}";
                var ip = string.Join(".", BitConverter.GetBytes(deviceInfo.nCurrentIp));
                Log.Information(
    ...
    {
        var errorMessage = $"[海康工业相机服务] SDK 异常回调: Type={nMsgType:X8}, SN: {_currentDeviceSerialNumber ?? "N/A"}";
    ...
    private void HandleImageCallback(IntPtr pstOutput, IntPtr pUser)
    {
        ...
        try
        {
            ...
            if (allCodeInfosForThisCallback.Any())
            {
                ...
                {
                    var combinedBarcode = string.Join(",", collectedBarcodes);
    ...
    private static bool Is1DBarcodeType(string typeId)
    {
        ...
        var known1DTypes = new[]
        {
            "Code128", "Code39", "EAN13", "UPCA", "UPCE", "Codabar", "Interleaved2of5"
        }; // Add more as needed
    ...
    private static BitmapSource? DrawBarcodeBordersOnImage(BitmapSource originalImage, IEnumerable<MVIDCodeReader.MVID_CODE_INFO> codeInfos)
    {
        ...
        try
        {
            ...
            using (var drawingContext = drawingVisual.RenderOpen())
            {
                ...
                Pen greenPen = new(Brushes.LimeGreen, 5); // 使用石灰绿，粗细为2
