...
public class AnttoWeightService(HttpClient httpClient, ISettingsService settingsService) : IAnttoWeightService
{
    ...
    public async Task<AnttoWeightResponse> UploadWeightAsync(AnttoWeightRequest request)
    {
        ...
        try
        {
            ...
            Log.Error("安通API称重数据上传失败，URL: {ApiUrl}, 状态码: {StatusCode}, 响应: {ResponseBody}", apiUrl, response.StatusCode,
                responseBody);
            // 尝试反序列化失败响应以获取更多信息
            var errorResponse = JsonConvert.DeserializeObject<AnttoWeightResponse>(responseBody);
            return errorResponse ?? new AnttoWeightResponse
                { Code = response.StatusCode.ToString(), Msg = "请求失败", ErrMsg = responseBody };
        }
