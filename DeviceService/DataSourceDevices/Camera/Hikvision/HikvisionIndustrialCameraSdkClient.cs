...
public class HikvisionIndustrialCameraSdkClient : ICameraService
{
    ...
    public HikvisionIndustrialCameraSdkClient()
    {
        ...
        Log.Information("已设置帧率限制：{FrameRate}fps，帧间隔：{FrameInterval}ms", TargetFrameRate,
            _frameInterval.TotalMilliseconds);
    ...
    private async Task ProcessImageDataAsync(IntPtr pImageData, int nImageSize)
    {
        ...
        finally
        {
            try
            {
                if (semaphore?.CurrentCount < _maxConcurrentProcessing) semaphore.Release();
            }
            catch (ObjectDisposedException)
            {
    ...
    private void OnImageCallback(IntPtr pImageData, int nImageSize)
    {
        try
        {
            ...
            // 将图像数据写入通道
            if (!_imageChannel.Writer.TryWrite((pImageData, nImageSize))) Log.Warning("图像通道已满，丢弃当前帧");
        }
        catch (Exception ex)
        {
