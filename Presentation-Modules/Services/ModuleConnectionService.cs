    ...
    public class ModuleConnectionService(ISettingsService settingsService) : IModuleConnectionService
    {
        ...
        public Task<bool> StartServerAsync(string ipAddress, int port)
        {
            try
            {
                ...
                _isRunning = true;
                Log.Information($"TCP服务器已启动，监听地址：{ipAddress}:{port}");
        ...
        private Task ProcessPackageNumberAsync(byte[] data)
        {
            try
            {
                ...
                _ = Task.Run(async () =>
                {
                    try
                    {
                        using var cts = new CancellationTokenSource(TimeSpan.FromMilliseconds(_config.MaxWaitTime));

                        try
                        {
                            await Task.Delay(_config.MaxWaitTime, cts.Token);
                            
                            // 超时处理
                            if (_waitingPackages.TryRemove(packageNumber, out _))
                            {
                                Log.Warning("包裹等待超时: 序号={PackageNumber}, 最大等待时间={MaxWaitTime}ms",
        ...
        private bool ValidatePackageBinding(ushort packageNumber, string? barcode)
        {
            ...
            {
                if (boundBarcode == barcode) return true;
            ...
            lock (_bindingLock)
            {
                if (!_packageBindings.TryAdd(packageNumber, barcode)) return false;
        ...
        public void OnPackageReceived(PackageInfo package)
        {
            try
            {
                ...
                foreach (var (packageNumber, waitInfo) in _waitingPackages)
                {
                    ...
                    // 检查时间范围
                    if (!(timeDiff >= _config.MinWaitTime) || !(timeDiff <= _config.MaxWaitTime)) continue;
                    // 验证包裹绑定关系
                    if (!ValidatePackageBinding(packageNumber, package.Barcode)) continue;
                ...
                if (!matchFound)
                {
                    Log.Warning("未找到符合时间范围的匹配包裹: {Barcode}", package.Barcode);
                }
            }
            catch (Exception ex)
            {
